...
pub struct RateLimit {
    /// MaxPerWindow (constant): the per-window acquire ceiling.
    ...
    pub max_clock: u64,
    /// count ∈ 0..MaxPerWindow: acquires granted in the current window.
    pub count: u64,
...
impl RateLimit {
    ...
    pub fn try_acquire(&mut self) -> (acquired: bool)
        ...
        ensures
            ...
            acquired == (old(self).window_expired() || old(self).count < old(self).max_per_window),
